"""
🤖 SMART MONEY F&O TRADING BOT v3.2
FULLY WORKING - API Response Keys Fixed!

✅ Correct API response parsing (timestamp not start_Time)
✅ Real data working (381 candles confirmed)
✅ Redis caching enabled
✅ All 15 symbols tracking
...
# ========================
# DHAN API HANDLER (FIXED!)
# ========================
class DhanAPI:
    """Dhan HQ API Integration - WORKING VERSION"""
    ...
    def get_historical_candles(self, security_id: int, segment: str, symbol: str) -> Optional[pd.DataFrame]:
        """Get historical candles - FIXED VERSION with correct response keys"""
        try:
            logger.info(f"📊 Fetching candles for {symbol} (security_id={security_id})")
            ...
            if response.status_code == 200:
                ...
                # ✅ FIX: Use 'timestamp' not 'start_Time'
                if 'timestamp' in data and 'open' in data and len(data.get('open', [])) > 0:
                    ...
                    logger.info(f"📊 {symbol}: Got {min_length} candles")
                    
                    # Create DataFrame
                    df = pd.DataFrame({
                        'timestamp': pd.to_datetime(timestamps[:min_length], unit='s'),
                    ...
                    if len(df) >= 20:
                        logger.info(f"✅ {symbol}: {len(df)} clean candles ready!")
                        return df
                    else:
                        logger.warning(f"⚠️ {symbol}: Only {len(df)} candles (need 20+)")
                        return None
                else:
                    logger.warning(f"⚠️ {symbol}: No valid data in response")
                    logger.info(f"📋 Response preview: {str(data)[:200]}")
                    return None
            else:
                logger.warning(f"⚠️ {symbol}: API returned status {response.status_code}")
                return None
    ...
    def get_option_chain(self, security_id: int, segment: str, expiry: str, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"⚠️ Option chain API returned status {response.status_code}")
...
- PCR: {oi_comp.get('pcr_old', 0):.2f} → {oi_comp.get('pcr_new', 0):.2f}
"""
...
"""
    ...
    async def send_startup_message(self):
        ...
        try:
            ...
            msg = f"""
🤖 <b>Smart Money Bot v3.2 Started!</b>
...
✅ Historical 5-min candles (FIXED!)
...
"""
    ...
    async def run(self):
        """Main bot loop"""
        logger.info("="*60)
        logger.info("🚀 SMART MONEY BOT v3.2 STARTING...")
        logger.info("="*60)
...
async def main():
    ...
    try:
        logger.info("="*60)
        logger.info("🚀 INITIALIZING SMART MONEY BOT v3.2")
        logger.info("="*60)
